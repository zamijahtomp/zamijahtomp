## Hi!!! 🤗

My name is Zamijah (zuh-my-jzuh) Shakeur-Tompkins, and I am a Senior Compsci major at Howard University in Washington, DC. I discovered a greater interest in game development this year after creating Boggle and Bison Trivia with my team for Software Engineering. Afterwards I took WEB101 with Codepath and realizing how much I love web development I figured I definitely want this to be my hobby if not my career. I've been programming games like Pong (I have like 5 versions of Pong now in different languages loll) since I was in middle school and with my newfound experience with web-dev I was able to start the creation of my own online gaming platform, reminiscent of the flash sites I would go to as a child. This summer I intend to have at least two games fully functioning, Pong and Battleship! 💫

😄 Pronouns: she/her/hers 🏳️‍🌈💖

💬 Ask me about:
- My plants 🌱
- My crochet projects 🧶


📫 How to reach me: Here are my Linkedin and Instagram if you want to reach out!
- 👥 Linkedin: https://www.linkedin.com/in/zamijah-shakeur-tompkins-b0b885297/
- 📸 Instagram: https://www.instagram.com/lovely_zamijah/

## 🕹️ Active Projects

### 🌸 Online Games by @lovely_zamijah (HTML5 / CSS3 / JavaScript / Cloudflare)
A nostalgic, Flash-inspired game portal hosting local two-player web games.
* **Current Games:** Pong 🏓, Battleship 🚢
* **Status:** Site is fully functional but games are still in development. Pong is functional but still needs some work, don't touch Battleship lol.
* **Check it out:** [🌸 Games by @lovely_zamijah](https://games.zamijah-shakeurtomp.workers.dev/)

### 👽 Alien: 2D (Godot Engine)
A separate, standalone desktop RPG focusing on deep storytelling and nostalgic 2D graphics and animations. Takes inspiration from the Alien franchise.
* **Status:** Core engine and prototype development.
* **Tech Stack:** Godot, GDScript
<!--
**zamijahtomp/zamijahtomp** is a ✨ _special_ ✨ repository because its `README.md` (this file) appears on your GitHub profile.

Here are some ideas to get you started:


- 🌱 I’m currently learning ...
- 👯 I’m looking to collaborate on ...
- 🤔 I’m looking for help with ...
- 🔭 I’m currently working on:
- 
- 
- ⚡ Fun fact: ...
-->
